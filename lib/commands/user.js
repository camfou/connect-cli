...
function registerUser (cli, options, done) {
  ...
  userCmd
    .handler(function (data, flags, done) {
      ...
      cli.log('  nvl user:byEmail [<email>] [--issuer | -i <issuer id>]')
                    ...
                    cli.prompt([
                      {
                        ...
                        choices: users.filter(Boolean).map(function (user) {
                          return {
      ...
      .handler(function (data, flags, done) {
          cli.issuers.prompt(flags['issuer'] || flags.i, function (err, issuer) {
              ...
              anvil.discover()
                                      ...
                                      cli.prompt([
                                          {
                                              ...
                                              choices: users.map(function (user) {
                                                  return {
                                                      name: (user.email  + ' ' +user.name || user.givenName + ' ' + user.familyName) + ' – ' + chalk.yellow(user._id),
                                                      value: user.email
                                                  }
                                              })
                                          }
                  ...
                  .then(function (userId) {
                      return anvil.users.getByEmail(userId, { token: anvil.tokens.access_token })
                  })
                  .then(function (data) {
                      cli.log.json(data)
                      done()
                    ...
                    cli.prompt([
                      {
                        ...
                        choices: users.map(function (user) {
                          return {
                            name: (user.givenName || user.familyName) + ' ' + user.givenName + ' ('+ user.email +')' + ' – ' + chalk.yellow(user._id),
                            value: user._id
                          }
                        })
                      }
            ...
            return new Promise(function (resolve, reject) {
              cli.prompt([
                ...
                },
                {
                  ...
                  name: 'clubmedId',
                  message: 'clubmedId',
                  value: flags['clubmedId'] || flags.c,
                  default: user.clubmedId,
                  trim: true
                }
              ], function (update) {
                resolve({
                    ...
                    cli.prompt([
                      {
                        ...
                        choices: users.map(function (user) {
                          return {
                            name: (user.name || user.givenName + ' ' + user.familyName) + ' – ' + user.email + ' - ' +chalk.yellow(user._id),
                            value: user._id
                          }
                        })
                      }
